...
// Devuelve la posición en el índice indicado
// NULL si la posicion no es válida
Lpos getPos(List l, int index){
	if(index<0 || index>=l->size)
		return NULL;
	Lpos auxPos;
	register int iter=0;
	for(auxPos=l->head; iter<index && auxPos!=NULL; ++iter, auxPos=auxPos->next);
	return auxPos;
}
